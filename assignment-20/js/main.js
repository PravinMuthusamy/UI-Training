$(document).ready(function () {
    ...
    //if notes present ,hide empty page sectionaq   
    if (keys.length) {
        $('.empty').hide()
    }
    ...
    keys.forEach(element => {
        ...
        $note.append($(`<p>${note.notesTxt}</p></div>`))
        if(note.imagUrl){ $note.children('p').css('line-height','1.62em') }
        $('#notesArea').append($note.css('background-color', note.selectedClr))
